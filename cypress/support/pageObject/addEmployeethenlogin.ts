...
export default  class AddEmployeePage{
    ...
    /*
addLeaveEntitlements(data:any){
return  cy.api({
    method: 'POST',
    url: `https://opensource-demo.orangehrmlive.com/web/api/v2/leave/leave-entitlements?limit=50&offset=0&empNumber=${data.empNumber}&leaveTypeId=${data.leaveTypeId}&fromDate=${data.fromDate}&toDate=${data.toDate}`,
    body:{
      employeeId: `${GenericfunctionHelper.genericRandomString(1000)}`,
      leaveType:  data.leaveType, // Specify the leave type
...
addLeaveEntitlements(data:any){
    return   cy.api({
    method: 'POST',
    url: "https://opensource-demo.orangehrmlive.com/web/api/v2/leave/leave-requests",
       body: {
        // Provide the leave request details
        employeeId:  "0276",
        ...
        fromDate: data.fromDate,
         ...
         type: data.type,
         comment: null
        // Additional leave request parameters as needed
      },
